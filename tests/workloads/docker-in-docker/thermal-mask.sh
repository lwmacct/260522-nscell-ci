#!/bin/sh
set -eu

_bundle=""
_runc_root=""
_mounted=0

__cleanup() {
  if [ -n "$_runc_root" ]; then
    runc --root "$_runc_root" delete -f nscell-thermal-mask >/dev/null 2>&1 || true
  fi
  if [ "$_mounted" -eq 1 ]; then
    umount "${_bundle}/rootfs" >/dev/null 2>&1 || true
  fi
  if [ -n "$_bundle" ]; then
    rm -rf "$_bundle"
  fi
}

__diagnostics() {
  uname -a
  printf 'cpu-count=%s\n' "$(nproc)"
  runc --version | sed -n '1,2p'
  if [ -d /sys/devices/system/cpu/cpu0/thermal_throttle ]; then
    echo 'thermal-throttle-sysfs=present'
  else
    echo 'thermal-throttle-sysfs=absent'
  fi
}

__write_bundle() {
  mkdir -p /var/lib/docker/overlay2
  _bundle="$(mktemp -d /var/lib/docker/overlay2/nscell-thermal-mask.XXXXXX)"
  _runc_root="/run/nscell-runc-thermal-mask-$$"
  trap __cleanup EXIT HUP INT TERM

  mkdir -p \
    "${_bundle}/lower/bin" \
    "${_bundle}/lower/dev" \
    "${_bundle}/lower/proc" \
    "${_bundle}/lower/sys/devices/system/cpu/cpu0/thermal_throttle" \
    "${_bundle}/lower/tmp" \
    "${_bundle}/upper" \
    "${_bundle}/work" \
    "${_bundle}/rootfs"
  cp /bin/busybox "${_bundle}/lower/bin/busybox"
  ln -s busybox "${_bundle}/lower/bin/sh"

  mount -t overlay overlay \
    -o "lowerdir=${_bundle}/lower,upperdir=${_bundle}/upper,workdir=${_bundle}/work" \
    "${_bundle}/rootfs"
  _mounted=1

  (
    cd "$_bundle"
    runc spec
    jq '
      .process.terminal = false |
      .process.args = ["/bin/sh", "-c", "printf \"%s\\n\" nscell-runc-thermal-mask-ok"] |
      .root.readonly = false |
      .mounts = [] |
      .linux.rootfsPropagation = "rprivate" |
      .linux.namespaces = [{"type": "pid"}, {"type": "mount"}, {"type": "uts"}] |
      .linux.maskedPaths = ["/sys/devices/system/cpu/cpu0/thermal_throttle"] |
      del(.linux.resources)
    ' config.json >config.json.next
    mv config.json.next config.json
    runc --root "$_runc_root" run nscell-thermal-mask
  )
}

__main() {
  case "${1:-}" in
    diagnostics)
      __diagnostics
      ;;
    run)
      __write_bundle
      ;;
    *)
      echo "usage: $0 {diagnostics|run}" >&2
      exit 2
      ;;
  esac
}

__main "$@"
